...
class Crawling_baseball(baseball.Database):
    ...
    def __init__(self):
        ...
        self.is_start = True
        self.end = None
        self.last_game_num_list = list()
        self.team_game_idx_dic = dict()
    ...
    def ready_by_round(self):
        ...
        # 지금 페이지에서 crawling 할건지 다음페이지로 넘길건지
        if self.is_start: 
            self.is_start = False
    ...
    def create_team_game_info(self):
        '''
        team_game_info table 데이터 전처리하는 함수
    ...
    def create_score_array(self):
        '''
        score_record table 데이터 전처리하는 함수
    ...
    def create_batter_array(self,home_away):
        '''
        batter_record table 데이터 전처리 하는 함수
    ...
    def create_pitcher_array(self, home_away):
        '''
        pitcher_record table 데이터 전처리하는 함수
...
c.is_start = True
#%%
last_game_num_list = c.fetch_sql('select total_game_num from team_info')

#%%
d.fetch_sql('select * from game_info')
#%%

#%%
c.ready_by_match(j,c.elements[4])
#%%
p.pitcher_raw_array = c.pitcher_array
p.set_pitcher_array()
zz = pd.DataFrame(p.pitcher_array)
#%%  
p.set_conn()
conn = p.conn
...
zzz = pd.read_sql(sql,conn)
conn.commit()
conn.close()
#%%
zzz
#%%
score_columns = ['idx', 'date', 'stadium', 'home_away', 'team_num', 'team',
                 'foe_num', 'foe', 'result', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6',
                 'x7', 'x8', 'x9', 'x10', 'x11', 'x12', 'r', 'h', 'e', 'b']
score_array = p.score_array
score_pd= pd.DataFrame(score_array,columns = score_columns)

engine = create_engine("mysql+pymysql://root:" + "dudrn1" + "@127.0.0.1/baseball",encoding = 'utf-8')
conn = engine.connect()
score_pd.to_sql(name='score',con = engine,if_exists ='append',index = False)
conn.close()

#%%
for i,score_ in enumerate(c.score_array):
    team_num = score[3].astype(np.int)
    team_game_num = game_num_array[team_num-1,1]
    

    #%%
z1 = pd.DataFrame(p.score_array)
z2 = pd.DataFrame(p.batter_array)
z3 = pd.DataFrame(p.pitcher_array)
#%%
p = baseball.Precleaning()
p.batter_raw_array = c.batter_array
p.pitcher_raw_array = c.pitcher_array
p.score_raw_array = c.score_array
p.set_score_array()
p.set_batter_array()
p.set_pitcher_array()

zz = pd.DataFrame(p.score_array)
zzz =pd.DataFrame(c.pitcher_array)
#%%

#%%
zzz = c.last_game_num_list[1][0]
'%03d' % (zzz +1)
#%%
